...
| `/workflows` | mixed   | `panel`, `modal`, `node`, `asset`, `execution`, `version`, `compare`, `diff`, `inputTab`, `outputTab`, `inputSource`, `editor`, `deepTab`, `deepOutputTab`, `regexPattern`, `regexFlags`, `action` | Workflow panels, node/asset modals, version compare filters and useful debug/editor state. |
