...
| `/workflows` | mixed   | `panel`, `modal`, `node`, `execution`, `version`, `compare`, `diff`, `inputTab`, `outputTab`, `inputSource`, `editor`, `deepTab`, `deepOutputTab`, `regexPattern`, `regexFlags`, `action` | Workflow panels, modals, version compare filters and useful debug/editor state. |
