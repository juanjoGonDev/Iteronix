...
export const parseSkillRunResponse = (value: unknown): SkillRunResponse => ({
  ...
  confidence: parseConfidenceScore(
    readRequiredRecord(value, "skillRun", "confidence"),
  ),
  evidenceReport: parseEvidenceReport(
    readRequiredRecord(value, "skillRun", "evidenceReport"),
  ),
  traceId: readRequiredString(value, "skillRun", "traceId"),
  usage: parseUsageRecord(readRequiredRecord(value, "skillRun", "usage")),
});

export const parseWorkflowRunResponse = (
  value: unknown,
): WorkflowRunResponse => {
  ...
  const checkpoint =
    checkpointValue === undefined
      ? undefined
      : {
          stage: readEnum(checkpointValue, "workflowCheckpoint", "stage", [
            "reviewer",
          ]),
  ...
  return checkpoint === undefined
    ? {
        ...
        steps: parseWorkflowSteps(
          readRequiredArray(record, "workflowRun", "steps"),
        ),
        final: parseSkillRunResponse(
          readRequiredRecord(record, "workflowRun", "final"),
        ),
      }
    : {
        status,
...
export const parseEvaluationRunResponse = (
  value: unknown,
): EvaluationRunResponse => {
  ...
  return {
    ...
    results: results.map((item) => {
      ...
      return {
        ...
        reasons: parseStringArray(
          readRequiredArray(record, "evaluationResult", "reasons"),
          "evaluationReasons",
        ),
      };
    }),
...
export const parseMemorySearchResults = (
  value: unknown,
  ...
  return items.map((item) =>
    parseMemorySearchResult(ensureRecord(item, "memoryItem")),
  );
};

export const parseWorkbenchHistoryState = (
  value: unknown,
): WorkbenchHistoryState => {
  ...
  return {
    runs: Array.isArray(runsValue)
      ? runsValue.map((item) => parseRunHistoryRecord(item))
      : [],
    evals: Array.isArray(evalsValue)
      ? evalsValue.map((item) => parseEvalHistoryRecord(item))
      : [],
  };
};
...
const parseRunHistoryRecord = (value: unknown): WorkbenchRunHistoryRecord => {
  ...
  const kind = readEnum(record, "workbenchRunRecord", "kind", [
    "skill",
    "workflow",
  ]);
  const reviewValue = readOptionalRecord(record, "review");
  const resultValue = readRequiredRecord(
    record,
    "workbenchRunRecord",
    "result",
  );
  const base = {
    ...
    memory: parseMemoryArray(
      readRequiredArray(record, "workbenchRunRecord", "memory"),
    ),
  };
  ...
  const review =
    reviewValue === undefined ? undefined : parseWorkflowReview(reviewValue);
...
const parseEvalHistoryRecord = (value: unknown): WorkbenchEvalHistoryRecord => {
  ...
  return {
    ...
    datasetPath: readRequiredString(
      record,
      "workbenchEvalRecord",
      "datasetPath",
    ),
    ...
    result: parseEvaluationRunResponse(
      readRequiredRecord(record, "workbenchEvalRecord", "result"),
    ),
  };
};

const parseSkillDescriptor = (
  value: Record<string, unknown>,
): SkillRunResponse["skill"] => {
  ...
  return {
    metadata: {
      ...
      tags: parseStringArray(
        readRequiredArray(metadata, "skillMetadata", "tags"),
        "skillTags",
      ),
    },
  };
...
const parseEvidenceReport = (
  ...
  retrievedSources: parseCitations(
    readRequiredArray(value, "evidenceReport", "retrievedSources"),
  ),
  confidence: parseConfidenceScore(
    readRequiredRecord(value, "evidenceReport", "confidence"),
  ),
  usage: parseUsageRecord(readRequiredRecord(value, "evidenceReport", "usage")),
});

const parseConfidenceScore = (
  value: Record<string, unknown>,
): ConfidenceScore => ({
  score: readRequiredNumber(value, "confidenceScore", "score"),
  label: readEnum(value, "confidenceScore", "label", ["low", "medium", "high"]),
  signals: parseStringArray(
    readRequiredArray(value, "confidenceScore", "signals"),
    "confidenceSignals",
  ),
});
...
const parseUsageRecord = (value: Record<string, unknown>): UsageRecord => ({
  ...
  completionTokens: readRequiredNumber(
    value,
    "usageRecord",
    "completionTokens",
  ),
  totalTokens: readRequiredNumber(value, "usageRecord", "totalTokens"),
  estimatedCostUsd: readRequiredNumber(
    value,
    "usageRecord",
    "estimatedCostUsd",
  ),
...
const parseWorkflowSteps = (
  ...
  value.map((item) => {
    ...
    return {
      ...
      status: readEnum(record, "workflowStep", "status", [
        "completed",
        "awaiting_approval",
      ]),
...
const parseMemoryArray = (
  value: ReadonlyArray<unknown>,
): ReadonlyArray<MemorySearchResult> =>
  value.map((item) =>
    parseMemorySearchResult(ensureRecord(item, "memorySearchResult")),
  );

const parseMemorySearchResult = (
  value: Record<string, unknown>,
): MemorySearchResult => {
  ...
  return {
    ...
    tags: parseStringArray(
      readRequiredArray(value, "memorySearchResult", "tags"),
      "memoryTags",
    ),
...
const parseWorkflowReview = (
  value: Record<string, unknown>,
): WorkflowReview => ({
  decision: readEnum(value, "workflowReview", "decision", [
    "approved",
    "denied",
  ]),
