...
describe("explorer state helpers", () => {
  it("sorts directories before files and flattens expanded nodes", () => {
    ...
    expect(
      flattenExplorerTreeNodes(tree).map((item) => item.node.path),
  ...
  it("merges loaded children into a directory and toggles expansion", () => {
    ...
    const expandedTree = mergeExplorerDirectoryChildren(
      initialTree,
      "src",
      childEntries,
    );
    ...
    expect(
      flattenExplorerTreeNodes(expandedTree).map((item) => item.node.path),
    ).toEqual(["src", "src/index.ts"]);
    expect(
      flattenExplorerTreeNodes(collapsedTree).map((item) => item.node.path),
    ).toEqual(["src"]);
  });
  ...
  it("applies global collapse and expand states to a loaded tree", () => {
    ...
    expect(
      flattenExplorerTreeNodes(collapsed).map((item) => item.node.path),
    ).toEqual(["src"]);
    expect(
      flattenExplorerTreeNodes(expanded).map((item) => item.node.path),
    ).toEqual(["src", "src/screens", "src/screens/Explorer.ts"]);
  });
  ...
  it("expands a specific loaded directory without toggling other nodes", () => {
    ...
    expect(
      flattenExplorerTreeNodes(revealed).map((item) => item.node.path),
    ).toEqual(["src", "src/screens", "src/screens/Explorer.ts"]);
  });
  ...
  it("filters the tree while preserving matching ancestors and derives language labels", () => {
    ...
    expect(
      flattenExplorerTreeNodes(filtered).map((item) => item.node.path),
    ).toEqual(["src", "src/Explorer.ts"]);
  ...
  it("manages open editor tabs, pinning and directional closing deterministically", () => {
    const opened = openExplorerFile(
      openExplorerFile(openExplorerFile([], "README.md"), "src/index.ts"),
    ...
    const closedLeft = closeExplorerFileTabsToLeft(
      pinned,
      "src/screens/Explorer.ts",
    );
    ...
    expect(closedRight).toEqual([{ path: "README.md", pinned: true }]);
  ...
  it("manages collapsed and hidden search result groups per file", () => {
    ...
    const collapsed = collapseExplorerSearchResultPath(
      [],
      "src/screens/Explorer.ts",
    );
    const expanded = collapseExplorerSearchResultPath(
      collapsed,
      "src/screens/Explorer.ts",
    );
    ...
    expect(
      isExplorerSearchResultCollapsed(collapsed, "src/screens/Explorer.ts"),
    ).toBe(true);
    expect(
      isExplorerSearchResultCollapsed(expanded, "src/screens/Explorer.ts"),
    ).toBe(false);
    expect(
      filterHiddenExplorerSearchResults(results, hidden).map(
        (entry) => entry.path,
      ),
    ).toEqual(["src/screens/Explorer.ts"]);
    expect(
      filterHiddenExplorerSearchResults(results, visibleAgain).map(
        (entry) => entry.path,
      ),
    ).toEqual(["src/screens/Explorer.ts", "README.md"]);
    expect(
      reconcileExplorerSearchResultPaths(
