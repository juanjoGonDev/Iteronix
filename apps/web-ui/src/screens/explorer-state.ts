...
export const mergeExplorerDirectoryChildren = (
  ...
  return nodes.map((node) =>
    node.path === directoryPath && node.kind === ExplorerFileEntryKind.Directory
      ...
      : {
          ...node,
          children: mergeExplorerDirectoryChildren(
            node.children,
            directoryPath,
            entries,
          ),
...
export const filterExplorerTreeNodes = (
  ...
  return nodes.flatMap((node) => {
    const filteredChildren = filterExplorerTreeNodes(
      node.children,
      normalizedQuery,
    );
...
export const closeExplorerFileTabsToLeft = (
  ...
  const normalizedPath = normalizeExplorerPath(path);
  const pivotIndex = openFiles.findIndex(
...
export const resolveNextExplorerActiveFilePath = (
  ...
  if (
    preferred.length > 0 &&
    openFiles.some((entry) => entry.path === preferred)
  ) {
    return preferred;
  }
  ...
  const activeIndex = openFiles.findIndex(
    (entry) => entry.path === normalizedActivePath,
  );
  if (activeIndex >= 0) {
    return openFiles[activeIndex]?.path ?? null;
  }

  const insertionIndex = findExplorerOpenFileInsertionIndex(
    openFiles,
...
export const reconcileExplorerSearchResultPaths = (
  ...
  const resultPaths = new Set(
    results.map((entry) => normalizeExplorerPath(entry.path)),
  );
...
export const readExplorerLanguageTheme = (
  path: string,
): ExplorerLanguageTheme => {
  const languageId = readExplorerLanguageId(path);
...
export const readExplorerTokenClassName = (kind: ExplorerTokenKind): string => {
...
const joinExplorerPreviewContent = (left: string, right: string): string => {
  if (left.length === 0) {
    return right;
  }
...
const highlightJsonLine = (
  ...
  if (lastIndex < line.length) {
    tokens.push(
      createHighlightToken(line.slice(lastIndex), ExplorerTokenKind.Plain),
    );
  }

  return tokens.length > 0
    ? tokens
    : [createHighlightToken("", ExplorerTokenKind.Plain)];
};
...
const readJsonTokenKind = (
  ...
  if (/^"(?:\\.|[^"])*"$/.test(value)) {
    ...
    return nextCharacter === ":"
      ? ExplorerTokenKind.Property
      : ExplorerTokenKind.String;
  }
