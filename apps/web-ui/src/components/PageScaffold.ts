    ...
    return createElement(
      ...
      [
        createElement("div", { className: "flex min-w-0 flex-col gap-2" }, [
          createElement("h1", { className: readPageIntroTitleClassName() }, [
            title,
          ]),
          description
            ? createElement(
                "p",
                { className: readPageIntroDescriptionClassName() },
                [description],
              )
            : "",
        ]),
  ...
  override render(): HTMLElement {
    ...
    return createElement(
      ...
      [
        createElement(
          "div",
          { className: "scrollbar-hide flex gap-8 overflow-x-auto pb-1" },
          [
            items.map((item) =>
              createElement(
                "button",
...
export const readPageFrameClassName = (className = ""): string =>
  joinClasses(
    "mx-auto flex w-full max-w-[1480px] flex-col gap-6 p-6",
    className,
  );
...
const createGlobalToastElement = (
  ...
  dismissButton.className =
    "ml-3 flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-current opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-current/30";
