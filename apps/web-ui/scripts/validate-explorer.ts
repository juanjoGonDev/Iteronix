...
const Selector = {
  ...
  SearchResultFile:
    '[data-testid="explorer-search-result-file-src-screens-explorer-ts"]',
  SearchResultMatch:
    '[data-testid="explorer-search-result-match-src-screens-explorer-ts-648"]',
  ...
  ExplorerNodeNestedFile:
    '[data-testid="explorer-node-src-screens-explorer-ts"]',
...
const FixtureTabPaths = Array.from(
  { length: 20 },
  (_, index) =>
    `workspace-validation-tab-${String(index + 1).padStart(2, "0")}.ts`,
...
async function validateDesktopExplorer(
  ...
  await page.goto(
    `${ValidationConfig.PreviewBaseUrl}${ValidationConfig.ExplorerRoute}`,
    {
  ...
  await waitForCondition(
    async () => {
      return page.evaluate(
        ...
        Selector.HighlightedLine,
        ValidationText.SearchResultLineNumber,
      );
    },
  ...
  if (rangeAfterBottomScroll === null) {
    throw new Error(
      "Preview range was not available after scrolling near the bottom.",
    );
  }
  ...
  if (treeScrollTop <= 0) {
    throw new Error(
      "Explorer tree surface did not become scrollable during validation.",
    );
  }
  await clickSelector(
    page,
    explorerNodeSelector(FixtureTabPaths.at(-1) ?? "tab-12.ts"),
  );
  await waitForSelector(
    page,
    explorerTabSelector(FixtureTabPaths.at(-1) ?? "tab-12.ts"),
  );
  await delay(200);
  await waitForCondition(
    async () => {
  ...
  await page.evaluate(
    (input: { selector: string; initialScrollLeft: number }) => {
      ...
      surface.scrollLeft =
        input.initialScrollLeft > 0 ? 0 : Math.min(surface.scrollWidth, 180);
    },
    {
    ...
    async () => {
      return page.evaluate(
        (input: { selector: string; initialScrollLeft: number }) => {
          ...
          return (
            surface instanceof HTMLElement &&
            surface.scrollLeft !== input.initialScrollLeft
          );
        },
        {
          selector: Selector.TabsScrollSurface,
  ...
  await rightClickTabAndSelectAction(
    page,
    Selector.TabReadme,
    "Close to the right",
  );
...
async function validateCompactExplorer(
  ...
  await waitForCondition(
    async () => {
      ...
      return (
        sidebarWidth !== null &&
        sidebarWidth <= ValidationConfig.CompactSidebarMaxWidth
      );
    },
...
async function dispatchSearchInputValue(
  page: Page,
  value: string,
): Promise<void> {
  await page.evaluate(
...
async function scrollPreviewSurfaceToProgress(
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    await page.evaluate(
      (input: { selector: string; ratio: number }) => {
        const surface = document.querySelector(input.selector);
        if (!(surface instanceof HTMLElement)) {
          return;
        }
        ...
        const nextScrollTop = Math.max(
          0,
          Math.floor(maxScrollTop * input.ratio),
        );
...
async function startExplorerStubServer(): Promise<
...
async function handleExplorerStubRequest(
  ...
  if (url.pathname === RequestPath.FilesRead) {
    ...
    respondJson(
      response,
      200,
      createFixtureFileReadResponse(content, {
...
async function rightClickTabAndSelectAction(
  ...
  await waitForCondition(
    async () => {
      return page.evaluate(
        (input: { menuSelector: string; actionLabel: string }) => {
