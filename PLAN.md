...
The active OpenSpec change `fix-workflow-live-execution-refresh` records completed implementation and a passed historical verification report for live execution persistence, refresh behavior, double-click handling, and SSE disconnect tolerance. It is still unarchived. Browser-validator recovery is committed at `e6dc42a`; GitHub CI run `30262692814` passed the Puppeteer validator and Docker-backed Playwright coverage.
