...
The archived OpenSpec change `fix-workflow-live-execution-refresh` records completed implementation and an authoritative passed verification report for live execution persistence, refresh behavior, double-click handling, and SSE disconnect tolerance. It was synced to the OpenSpec source of truth and archived on 2026-07-28 after the bound native review gate passed. Browser-validator recovery is committed at `e6dc42a`; GitHub CI run `30262692814` passed the Puppeteer validator and Docker-backed Playwright coverage.
